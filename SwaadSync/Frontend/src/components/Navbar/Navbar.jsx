...
function Navbar() {
  const [isDark, setIsDark] = useState(
    document.body.classList.contains("ss-dark")
  );
  const location = useLocation();
  const iconColor = isDark ? "#FFAF45" : "#111";
...
function SettingsDropdown({ isDark, setIsDark }) {
  ...
  const handleThemeChange = (checked) => {
    if (checked) document.body.classList.add("ss-dark");
    else document.body.classList.remove("ss-dark");
  ...
  return (
    <div className="ss-navbar-bottom ss-settings-dropdown" ref={ref}>
      ...
      <span className="ss-navbar-label">Settings</span>
      {open && (
