...
const RecipeModal = ({ recipe, open, onClose, onLike, onSave, onComment, liked, saved, comments }) => {
  ...
  return (
    <div className="ss-modal-overlay" onClick={onClose}>
      <div className="ss-modal-content" onClick={e => e.stopPropagation()}>
        ...
        <img src={recipe.images?.[0]?.url} alt={recipe.name} className="ss-modal-img" />
        <h2 className="ss-modal-title">{recipe.name}</h2>
        <p className="ss-modal-desc">{recipe.description}</p>
        <div className="ss-modal-cuisine">Cuisine: {recipe.cuisine || "Unknown"}</div>
